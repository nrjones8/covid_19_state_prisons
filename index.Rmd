...
### Figure 1. A State-Level Map of the USA Showing 1) Positive COVID-19 Tests Among Incarcerated People and Correctional Staff, and 2) Intensive Care Unit Beds
...
```{r, layout="l-page", fig.height = 6.5}
...
state_data <-
  ...
  mutate(state = as.factor(state),
         scrape_date = lubridate::ymd(scrape_date))
...
max_date <- max(facility_data$scrape_date[!is.na(facility_data$scrape_date)])

facility_data <- facility_data %>%
  filter(scrape_date %in% max_date) %>%
  arrange(desc(inmates_positive),
          state,
          facilities) %>%
