...
fns_list <- list(alabama = get_ala_covid_data,alaska = get_alaska_covid_data,
                 ...
                 south_carolina = get_sc_covid_data,texas = get_texas_covid_data,virginia = get_virginia_covid_data,
                 washington = get_washington_covid_data)
urls_to_scrape<- data_in_goog_sheet %>% 
  filter(scraped_binary == 1,!state %in% c("Massachusetts","Connecticut")) %>%
  pull(link) %>% 
  map(~safe_get(.))
...
states_with_cc_facility <- jails_data[c("alabama","arizona","florida","georgia","illinois","kansas","pennsylvania","south_carolina","virginia","washington","louisiana","texas")]
...
summary_states <- states_with_cc_facility[!names(states_with_cc_facility) %in% c("alabama","arizona","washington","louisiana")] %>% 
  map(~select(.,state,facilities,inmates_positive)) %>% 
  reduce(bind_rows) %>% 
